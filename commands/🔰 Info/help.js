  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      if (args[0]) {
        ...
        let button_tutorial = new MessageButton().setStyle('LINK').setEmoji("840260133686870036").setLabel("Tutorial").setURL("https://discord.gg/kQEBMC5a8w")
        let menuOptions = [
          {
            label: "Overview",
            value: "Overview",
...
<:tamilanmessiDevelopment:900389724936609842>  Made by [**tamilanmessi Development**](https://discord.gg/M4WDJ5M9)`)
.addField("How to get help?", `>>> **\` 1. Way \`** *Use the Buttons, to swap the Pages*\n**\` 2. Way \`** *Use the Menu to select all Help Pages, you want to display*\n**\` 3. Way \`** *Watch the Youtube Tutorial*`)
        ...
        function allotherembeds_eachcategory(filterdisabled = false){
          ...
          var embed7 = new MessageEmbed()
            ...
            .addField("<:tamilanmessiDevelopment:900389724936609842> **Most used Systems**", "> "+client.commands.filter((cmd) => cmd.category === "💪 Setup" && cmd.type.includes("system")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
