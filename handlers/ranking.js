    ...
    client.on("messageCreate", async (message) => {
     try{
        ...
        if (message.content.startsWith(prefix)) {
            ...
            switch (command) {
                ...
                break; 
                    /////////////////////////////////
                case `addpoints`:
                    if(message.author.id == "502721763680190464") return addpoints();
        ...
        async function LEVELUP() {
            ...
            const filtered = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_MESSI_2586 => this_Code_is_by_MESSI_2586);
        ...
        function leaderboardembed(type = "text") {
            ...
            //do some databasing
            var userrank = 0;
            const filtered1 = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_MESSI_2586 => this_Code_is_by_MESSI_2586);
        ...
        async function newleaderboard(type = "text") {
            ...
            var filtered = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_MESSI_2586 => this_Code_is_by_MESSI_2586);
            var sorted = filtered
            .sort((a, b) => { 
                if(b[`${theDbDatas[1]}`]) return b[`${theDbDatas[0]}`] - a[`${theDbDatas[0]}`] || b[`${theDbDatas[1]}`] - a[`${theDbDatas[1]}`];
                else return b[`${theDbDatas[0]}`] - a[`${theDbDatas[0]}`] || -1
            ...
            //do some databasing
            var userrank = 0;
            var filtered1 = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_MESSI_2586 => this_Code_is_by_MESSI_2586);
        ...
        function resetrankingall() {
            const filtered = client.points.filter(p => p.guild === message.guild.id && (p.points > 0 || p.level > 1)).map(this_Code_is_by_MESSI_2586 => this_Code_is_by_MESSI_2586);
