...
module.exports = (client) => {
  ...
  const maxTicketsAmount = 25;
  for(let i = 1; i<=maxTicketsAmount; i++) {
    //define the apply system variable
    let filename = `ticket${i}`;
    let systempath = `ticketsystem${i}`;
    let ticketspath = `tickets${i}`; 
    let idpath = `ticketid${i}`; 
    let tickettypepath = `ticket-setup-${i}`; 
    ...
    client.on("interactionCreate", async (interaction) => {
      ...
      let channelname = ticket.defaultname.replace("{member}", user.username).replace("{count}", ticketamount).replace(/\s/igu, "-").substr(0, 31);
      ...
      guild.channels.create(channelname.substr(0, 31), optionsData).then(async ch => {
        ...
        if(ticket.claim.enabled){
          var claimEmbed = new MessageEmbed()
            ...
            .setAuthor(client.getAuthor(`A Staff Member will claim the Ticket soon!`, "https://cdn.discordapp.com/emojis/833101350623117342.gif?size=44", "https://discord.gg/tamilanmessi"))
